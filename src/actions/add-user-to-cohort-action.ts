...
export async function inviteUserToCohort(cohortId: number, formData: FormData) {
  ...
  try {
    ...
    const { data: inviteData, error: inviteError } = await supabaseAdmin.inviteUserByEmail(email, {
      data: { app_role: "user" },
      redirectTo: "/welcome",
    })
    ...
    if (inviteError?.status === 400 || inviteError?.code === "email_exists") {
      const { id: existingUserId } = await queryUserByEmail(supabase, { email })
    ...
    if (userId) {
      ...
      return { success: true, message: 'User invited and added to cohort successfully.' }
    } else {
